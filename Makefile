...
clean: check gcp-down
	tilt down
	rm almsc-kubeconfig.yaml

.PHONY: gcp-up
gcp-up: require-gcloud
	KUBECONFIG=almsc-kubeconfig.yaml gcloud container clusters create --project ${GCP_PROJECT} "downstream-${USER}" --region ${GCP_REGION} \
		...
		--no-enable-autorepair --max-surge-upgrade 1 --max-unavailable-upgrade 0 --enable-shielded-nodes --labels=used_by=${USER} || true

.PHONY: gcp-down
gcp-down: require-gcloud
	gcloud container clusters delete "downstream-${USER}" --region ${GCP_REGION} --project ${GCP_PROJECT} -q || true
